...
<body>
    ...
    if(isset($_SESSION["user"])) {
        // Fetch the count of wishlist items from the database
        $uid = $_SESSION['uid'];
        $wishlist_count_query = "SELECT COUNT(*) AS count FROM wishlist WHERE uid='$uid'";
        $wishlist_count_result = mysqli_query($con, $wishlist_count_query);
        $wishlist_count_row = mysqli_fetch_assoc($wishlist_count_result);
        $wishlist_count = $wishlist_count_row['count'];
        echo $wishlist_count; // Display the count
    } else {
        echo '0'; // Display 0 when user is not logged in
    }
    ?>
</div>
    ...
    ?>
                            ...
                            <?php echo $_SESSION["name"]; ?><br><a href="MyAcount.php"><small>My Account</small> 
                            </a>
                            <?php } else {?>
